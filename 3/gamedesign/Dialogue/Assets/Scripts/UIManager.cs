...
public class UIManager : MonoBehaviour
{
    ...
    void UpdateUI(VD.NodeData data)
    {
        ...
        else  //If it's an NPC Node, let's just update NPC's text and sprite
        {
            ...
            if (data.tag.Length > 0)
                ...
                NPC_label.text = VD.assigned.alias + mood.GetMoodSuffix();
    ...
    bool PreConditions(VIDE_Assign assigned)
    {
        ...
        if (VD.isActive)
        {
            if (data.extraVars.ContainsKey("moodBranch"))
            {
                int[] nodeIDs = VD.ToIntArray((string) data.extraVars["moodBranch"]);
                int id = (int) mood.MoodType;
                VD.SetNode(nodeIDs[id]);
                return true;
            }
        }
